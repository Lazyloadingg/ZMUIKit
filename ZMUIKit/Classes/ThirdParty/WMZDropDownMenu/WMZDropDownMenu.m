...
- (void)addBoder{
    [self.titleBtnArr enumerateObjectsUsingBlock:^(WMZDropMenuBtn *btn, NSUInteger idx, BOOL * _Nonnull stop) {
        if (idx != self.titleBtnArr.count-1) {
             [WMZDropMenuTool viewPathWithColor:[UIColor redColor] PathType:MenuShadowPathRight PathWidth:1 heightScale:0.5 button:btn];
...
- (UITableViewCell *)tableView:(WMZDropTableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([tree isKindOfClass:[WMZDropTree class]]) {
        [cell update:tree.name isOk:tree.isSelected cellHeight:tree.cellHeight];
...
//        ZMLog(@"origin.x=%f",cell.textLabel.frame.origin.x);
    ...
    if (path.UIStyle == MenuUICollectionRangeTextField){
        ...
        return cell;
         ...
         if ([tree isKindOfClass:[WMZDropTree class]]) {
             ...
             cell.btn.layer.borderColor = [UIColor colorRed2].CGColor;
...
#pragma -mark 关闭方法
- (void)closeView{
    [self closepopView:true];
}
/// 关闭
/// @param isClose 是否有关闭动画
- (void)closepopView:(BOOL) isClose{
    if (self.keyBoardShow) {
...
- (void)closeAction:(BOOL) isClose{
     WMZDropIndexPath *currentDrop = [self getTitleFirstDropWthTitleBtn:self.selectTitleBtn];
      ...
      //动画
      MenuWeakSelf(self)
      float menuAnimateTime = 0;
      if (isClose) {
          menuAnimateTime = menuAnimalTime;
      }
      [self hideAnimal:currentDrop.hideAnimalStyle view:self.dataView durtion:menuAnimateTime block:^{
          MenuStrongSelf(weakObject)
...
- (void)cellTap:(WMZDropIndexPath*)dropPath data:(NSArray*)arr indexPath:(NSIndexPath*)indexPath{
    ...
    if (dropPath.editStyle == MenuEditOneCheck) {
        ...
        if (dropPath.tapClose&&tree.isSelected) {
            ...
            if (self.delegate && [self.delegate respondsToSelector:@selector(replaceTitlemenu:didSelectIndexPath:)]) {
                replaceTitle = [self.delegate replaceTitlemenu:self didSelectIndexPath:indexPath];
            }
            NSString *selectTitle;
            if(replaceTitle){
                selectTitle = replaceTitle;
            }else{
                selectTitle = tree.name;
            }
            [self changeTitleConfig:@{@"name":selectTitle} withBtn:self.selectTitleBtn];
            [self closepopView:false];
        }
