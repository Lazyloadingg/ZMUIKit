...
-(void) createUI{
    ...
    [self.contentView addSubview: self.okImgView];
}
-(void) resetCell{
    [self.textLbl removeFromSuperview];
    [self.okImgView removeFromSuperview];
    [self createUI];
}
-(UILabel *) textLbl{
    if (!_textLbl) {
        _textLbl = [[UILabel alloc] init];
        _textLbl.font = [UIFont zm_font14pt:DYFontBoldTypeRegular];
        _textLbl.textColor = [UIColor colorC5];
    }
    return _textLbl;
}
-(UIImageView *) okImgView{
    if (!_okImgView) {
        ...
        _okImgView.image = [UIImage bundleImage:@"menu_check"];
        _okImgView.contentMode = UIViewContentModeScaleAspectFit;
    }
    return _okImgView;
}
-(void) update:(NSString *) text isOk:(BOOL) isOK cellHeight:(float) cellHeight{
    [self resetCell];
    self.textLbl.text = text;
    float originX = self.textLabel.frame.origin.x;
    if (isOK) {
        self.okImgView.frame = CGRectMake(originX + 25, (cellHeight-20)/2, 20, 20);
        self.okImgView.hidden = false;
        self.textLbl.frame = CGRectMake(originX + 25 + 20 + 5, 0, 200, cellHeight);
    }
    else{
        self.textLbl.frame = CGRectMake(originX + 25, 0, 200, cellHeight);
        self.okImgView.frame = CGRectMake(originX + 25, (cellHeight-20)/2, 20, 20);
        self.okImgView.hidden = true;
    }
}
@end
...
- (void)layoutSubviews{
    [super layoutSubviews];
     ...
     self.btn.layer.cornerRadius = self.btn.bounds.size.height/2.0;
    
}
- (UIButton *)btn{
    if (!_btn) {
        _btn = [UIButton buttonWithType:UIButtonTypeCustom];
        _btn.titleLabel.font = [UIFont zm_font14pt:DYFontBoldTypeRegular];
    }
    return _btn;
}
@end
...
- (UILabel *)textLa{
    if (!_textLa) {
        _textLa = [UILabel new];
        _textLa.font = [UIFont zm_font15pt:DYFontBoldTypeMedium];
        _textLa.numberOfLines = 0;
        _textLa.textColor = [UIColor colorC5];
    }
    return _textLa;
}
...
- (UIButton *)resetBtn{
    if (!_resetBtn) {
        ...
        _resetBtn.titleLabel.font = [UIFont zm_font15pt:DYFontBoldTypeRegular];
...
- (UIButton *)confirmBtn{
    if (!_confirmBtn) {
        ...
        _confirmBtn.titleLabel.font =  [UIFont zm_font15pt:DYFontBoldTypeRegular];//[UIFont systemFontOfSize:15.0];
    }
    return _confirmBtn;
}
@end
