...
- (void)showLine:(NSDictionary*)config
{
    if (self.line == nil||[[self subviews] indexOfObject:self.line]==NSNotFound) {
        CGRect frame = CGRectMake((self.frame.size.width - 30)/2, self.frame.size.height-5, 30, 2);
        self.line = [[UIView alloc] initWithFrame:frame];
        self.line.backgroundColor = [UIColor colorRed2];//[UIColor menuColorGradientChangeWithSize:self.line.frame.size direction:MenuGradientChangeDirectionLevel startColor:MenuColor(0xf92921) endColor:MenuColor(0xffc1bd)];
