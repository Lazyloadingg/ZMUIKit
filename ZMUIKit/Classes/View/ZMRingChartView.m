...
typedef NS_ENUM(NSUInteger, ZMRingChartViewTouchStatus) {
    ZMRingChartViewTouchStatusBegan,
    ZMRingChartViewTouchStatusMoved,
    ZMRingChartViewTouchStatusEnded,
    ZMRingChartViewTouchStatusCancelled,
};
...
@property (nonatomic, assign) DYRingDistributionQuadrant quadrant;

@end

@interface ZMRingChartView ()
...
@implementation ZMRingChartView
@dynamic center;
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    [self didSelectWithPoint:point touchStatus:ZMRingChartViewTouchStatusEnded];
}
...
- (void)didSelectWithPoint:(CGPoint)point touchStatus:(ZMRingChartViewTouchStatus)touchStatus {
    ...
    [self.dataArr enumerateObjectsUsingBlock:^(SARingDistributionModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([obj.bezierPath containsPoint:point] && ![rectPath containsPoint:point]) {
            ...
            switch (touchStatus) {
                case ZMRingChartViewTouchStatusBegan:
                {
                    
                }
                    break;
                case ZMRingChartViewTouchStatusMoved:
                {
                    
                }
                    break;
                case ZMRingChartViewTouchStatusEnded:
                {
...
- (void)setMiddlePointWithAngle:(CGFloat)middleAngle index:(NSInteger)index {
    ...
    if (middleAngle >= -M_PI_2 && middleAngle <= 0) {//第一象限
        ...
        model.quadrant = DYRingDistributionQuadrantFirst;
        ...
        model.point = minPieCenter;
        model.quadrant = DYRingDistributionQuadrantFourth;
    }else if (middleAngle >= M_PI_2 && middleAngle <= 2*M_PI_2) {//第三象限
        CGFloat angle = M_PI - middleAngle;
        minPieCenter = CGPointMake(self.center.x - cosf(angle)*a, self.center.y + sinf(angle)*a);
        model.point = minPieCenter;
        model.quadrant = DYRingDistributionQuadrantThird;
    }else {//第二象限
        CGFloat angle = middleAngle - M_PI;
        minPieCenter = CGPointMake(self.center.x - cosf(angle)*a, self.center.y - sinf(angle)*a);
        model.point = minPieCenter;
        model.quadrant = DYRingDistributionQuadrantSecond;
    }
}
