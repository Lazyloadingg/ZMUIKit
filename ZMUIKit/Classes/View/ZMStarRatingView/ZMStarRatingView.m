...
@implementation ZMStarRatingView
...
- (void)drawRect:(CGRect)rect {
    ...
    CGFloat availableWidth = rect.size.width - (_minSpacing * (_maximumValue - 1)) - 2;
    ...
    for (int idx = 0; idx < _maximumValue; idx++) {
        CGPoint center = CGPointMake(cellWidth*idx + cellWidth/2 + _minSpacing*idx + 1, rect.size.height/2);
...
- (CGSize)intrinsicContentSize {
    CGFloat height = 44.f;
    return CGSizeMake(_maximumValue * height + (_maximumValue-1) * _minSpacing, height);
}
