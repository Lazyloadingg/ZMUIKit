...
#define kZMIndexViewSpace (self.configuration.indexItemHeight + self.configuration.indexItemsSpace)
#define kZMIndexViewMargin ((self.bounds.size.height - kZMIndexViewSpace * self.dataSource.count) / 2)
#define kZMIndexViewInsetTop (self.translucentForTableViewInNavigationBar ? UIApplication.sharedApplication.statusBarFrame.size.height + 44 : 0)
...
- (void)layoutSubviews {
    ...
    CGFloat space = kZMIndexViewSpace;
    CGFloat margin = kZMIndexViewMargin;
    ...
    for (int i = 0; i < self.subTextLayers.count; i++) {
        ...
        textLayer.frame = CGRectMake(self.bounds.size.width - self.configuration.indexItemRightMargin - self.configuration.indexItemWidth, SCGetTextLayerCenterY(section, margin, space) - self.configuration.indexItemHeight / 2, self.configuration.indexItemWidth, self.configuration.indexItemHeight);
    }
...
//        if ([textLayer.string isStringType:DYHasChineseStringType]) {
//            textLayer.fontSize = self.configuration.indexItemHeight * 0.7;
//        }else {
            textLayer.fontSize = self.configuration.indexItemHeight * 0.7;
//        }
    ...
    if (self.subTextLayers.count == 0) {
        ...
        self.currentSection = self.searchLayer ? ZMIndexViewSearchSection : 0;
    } else {
...
{
    NSInteger currentSection = ZMIndexViewInvalidSection;
    if (self.delegate && [self.delegate respondsToSelector:@selector(sectionOfIndexView:tableViewDidScroll:)]) {
        ...
        if ((currentSection >= 0 && currentSection != ZMIndexViewInvalidSection)
            || currentSection == ZMIndexViewSearchSection) {
            self.currentSection = currentSection;
            return;
        }
    }
    
    CGFloat insetTop = kZMIndexViewInsetTop;
...
{
    if ((self.currentSection < 0 && self.currentSection != ZMIndexViewSearchSection)
        || self.currentSection >= (NSInteger)self.subTextLayers.count) {
        return;
    }
    
    CGFloat insetTop = kZMIndexViewInsetTop;
    if (self.currentSection == ZMIndexViewSearchSection) {
...
- (void)showIndicator:(BOOL)animated
{
    ...
    textLayer.alignmentMode = kCAAlignmentCenter;
    if (self.configuration.indexViewStyle == ZMIndexViewStyleDefault) {
...
- (BOOL)beginTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event
{
    ...
    NSInteger currentPosition = SCPositionOfTextLayerInY(location.y, kZMIndexViewMargin, kZMIndexViewSpace);
...
- (void)setCurrentSection:(NSInteger)currentSection
{
    ...
    if ((currentSection < 0 && currentSection != ZMIndexViewSearchSection)
...
- (UILabel *)indicator
{
    if (!_indicator) {
        ...
        switch (self.configuration.indexViewStyle) {
            ...
            case ZMIndexViewStyleCenterToast:
            {
