...
@interface ZMImageEditorActionView ()

@property (nonatomic, strong) UIView *line;

@end

@implementation ZMImageEditorActionView
...
- (void)buttonAction:(UIButton *)button {
    if ([self.delegate respondsToSelector:@selector(actionView:didClickButton:atIndex:)]) {
        [self.delegate actionView:self didClickButton:button atIndex:button.tag];
    }
}
...
- (UIButton *)rotateButton {
    if (!_rotateButton) {
        ...
        UIImage *image = [UIImage zm_kitImageNamed:@"zm_kit_ic_rotate_90_degrees_ccw"];
...
- (UIButton *)cancelButton {
    if (!_cancelButton) {
        ...
        [_cancelButton setTitleColor:[UIColor colorWhite1] forState:UIControlStateNormal];
        _cancelButton.titleLabel.font = [UIFont zm_font16pt:DYFontBoldTypeRegular];
        _cancelButton.tag = 1;
...
- (UIButton *)originButton {
    if (!_originButton) {
        ...
        [_originButton setTitleColor:[UIColor colorWhite1] forState:UIControlStateNormal];
        _originButton.titleLabel.font = [UIFont zm_font16pt:DYFontBoldTypeRegular];
        _originButton.tag = 2;
...
- (UIButton *)finishButton {
    if (!_finishButton) {
        ...
        [_finishButton setTitleColor:[UIColor colorWhite1] forState:UIControlStateNormal];
        _finishButton.titleLabel.font = [UIFont zm_font16pt:DYFontBoldTypeRegular];
        _finishButton.tag = 3;
