...
-(void)addSectionCornerWithTableView:(UITableView *)tableView indexPath:(NSIndexPath *)indexPath cornerViewframe:(CGRect)frame cornerRadius:(CGFloat)cornerRadius{
    ...
    if (rows > 1) {
        ...
        if (!self.fullLay) {
            UIBezierPath *maskPath = [UIBezierPath bezierPathWithRoundedRect:self.cornerV.bounds byRoundingCorners:UIRectCornerBottomLeft | UIRectCornerBottomRight | UIRectCornerTopLeft |  UIRectCornerTopRight cornerRadii:CGSizeMake(cornerRadius, cornerRadius)];
            CAShapeLayer *maskLayer = [[CAShapeLayer alloc] init];
            maskLayer.frame = self.cornerV.bounds;
            maskLayer.path = maskPath.CGPath;
            self.fullLay = maskLayer;
        }
        self.cornerV.layer.mask = self.fullLay;
    }
}
...
-(void)setFullLay:(CAShapeLayer *)fullLay{
    objc_setAssociatedObject(self, @selector(fullLay), fullLay, OBJC_ASSOCIATION_RETAIN);
}
-(CAShapeLayer *)fullLay{
    return objc_getAssociatedObject(self, _cmd);
}
@end
