...
#define MBMainThreadAssert() NSAssert([NSThread isMainThread], @"DYProgressHUD needs to be accessed on the main thread.");

CGFloat const DYProgressMaxOffset = 1000000.f;
...
@interface DYProgressHUDRoundedButton : UIButton
@end
...
+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    DYProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    DYProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (DYProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            DYProgressHUD *hud = (DYProgressHUD *)subview;
...
- (void)commonInit {
    ...
    _animationType = DYProgressHUDAnimationFade;
    _mode = DYProgressHUDModeIndeterminate;
    _margin = 20.0f;
    _opacity = 1.f;
...
- (void)animateIn:(BOOL)animatingIn withType:(DYProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == DYProgressHUDAnimationZoom) {
        type = animatingIn ? DYProgressHUDAnimationZoomIn : DYProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == DYProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == DYProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == DYProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == DYProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
...
- (void)done {
    ...
    DYProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<DYProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    DYBackgroundView *backgroundView = [[DYBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = DYProgressHUDBackgroundStyleSolidColor;
    ...
    DYBackgroundView *bezelView = [DYBackgroundView new];
    ...
    UIButton *button = [DYProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[DYRoundProgressView class]];

    DYProgressHUDMode mode = self.mode;
    if (mode == DYProgressHUDModeIndeterminate) {
    ...
    else if (mode == DYProgressHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[DYBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == DYProgressHUDModeDeterminate || mode == DYProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
            // Update to determinante indicator
            [indicator removeFromSuperview];
            indicator = [[DYRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == DYProgressHUDModeAnnularDeterminate) {
            [(DYRoundProgressView *)indicator setAnnular:YES];
        }
    } 
    else if (mode == DYProgressHUDModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == DYProgressHUDModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
#pragma clang diagnostic pop
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[DYProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[DYProgressHUD class]]];
#endif
        
        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[DYRoundProgressView class]]) {
        DYRoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [DYRoundProgressView appearanceWhenContainedIn:[DYProgressHUD class], nil];
#else
        appearance = [DYRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[DYProgressHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((DYRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((DYRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[DYBarProgressView class]]) {
        DYBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [DYBarProgressView appearanceWhenContainedIn:[DYProgressHUD class], nil];
#else
        appearance = [DYBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[DYProgressHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            ((DYBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((DYBarProgressView *)indicator).lineColor = color;
        }
    } else {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
    DYBackgroundView *bezelView = self.bezelView;
...
- (void)setMode:(DYProgressHUDMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == DYProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
@implementation DYRoundProgressView
...
@implementation DYBarProgressView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_7_0) {
            _style = DYProgressHUDBackgroundStyleBlur;
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
            ...
            _style = DYProgressHUDBackgroundStyleSolidColor;
...
- (void)setStyle:(DYProgressHUDBackgroundStyle)style {
    if (style == DYProgressHUDBackgroundStyleBlur && kCFCoreFoundationVersionNumber < kCFCoreFoundationVersionNumber_iOS_7_0) {
        style = DYProgressHUDBackgroundStyleSolidColor;
    }
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
...
- (void)updateForBackgroundStyle {
    DYProgressHUDBackgroundStyle style = self.style;
    if (style == DYProgressHUDBackgroundStyleBlur) {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == DYProgressHUDBackgroundStyleBlur) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
            self.backgroundColor = self.color;
        } else {
#if !TARGET_OS_TV
...
@implementation DYProgressHUD (Deprecated)

#pragma mark - Class

+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
    NSArray *huds = [DYProgressHUD allHUDsForView:view];
    for (DYProgressHUD *hud in huds) {
        hud.removeFromSuperViewOnHide = YES;
        [hud hideAnimated:animated];
    }
    return [huds count];
...
- (void)showAnimated:(BOOL)animated whileExecutingBlock:(dispatch_block_t)block onQueue:(dispatch_queue_t)queue completionBlock:(nullable DYProgressHUDCompletionBlock)completion {
...
- (BOOL)dimBackground {
    DYBackgroundView *backgroundView = self.backgroundView;
    UIColor *dimmedColor =  [UIColor colorWithWhite:0.f alpha:.2f];
    return backgroundView.style == DYProgressHUDBackgroundStyleSolidColor && [backgroundView.color isEqual:dimmedColor];
}

- (void)setDimBackground:(BOOL)dimBackground {
    MBMainThreadAssert();
    self.backgroundView.style = DYProgressHUDBackgroundStyleSolidColor;
...
@implementation DYProgressHUDRoundedButton
